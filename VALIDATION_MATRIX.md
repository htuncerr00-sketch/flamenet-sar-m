...
| F-15 | **Cache concurrency** — mutex between sensor\_i2c\_task and telemetry\_task | `PASS [host-sim]` 49.9M reads × 7.5M writes, 0 torn reads (pthread mutex on x86) | `PENDING` verify FreeRTOS xSemaphoreTake latency <125 ns on real chip | `N/A` | `PENDING` 30 min at full rate on ESP32 | `N/A` | pthread on x86 ≠ FreeRTOS on ESP32. Priority inversion risk **RESOLVED 2026-05-27**: sensor priority raised to 12 (above telemetry at 10) — producer above consumer, no mutex contention possible from below. See TASK\_SCHEDULING\_AUDIT.md | `test_race_cache.c` 49.9M stress; FreeRTOS priority audit |
...
| ✅ ~~**P0-B**~~ | ~~FreeRTOS task priority inversion: telemetry(10) > sensor(9)~~ **RESOLVED 2026-05-27** — sensor raised to prio 12, safety_monitor raised to prio 3 and pinned to Core 1. See TASK\_SCHEDULING\_AUDIT.md | ~~Telemetry may busy-wait on mutex while sensor preempted~~ — **producer now structurally above consumer; no reactive mechanism needed** |
