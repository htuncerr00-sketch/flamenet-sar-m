/*
 * main/sensor_pipeline.c — Faz 19B Real Sensor Pipeline
 * ==========================================================
 ...
 *
 * Logical → wire field mapping:
 *   T_N        ← computed/external (HX711 hookup is Faz 19D-or-later;
 *                here we keep last-known-good or fall back to synthetic
 *                15 N nominal so PC validation still gets a value)
 *   current_A  ← INA226 current (signed; covers motor regen)
 *   vib_x,y,z  ← MPU6050 accel_g[0..2] in g
 *   temp_K     ← NTC + Steinhart-Hart
 *   rpm        ← computed/external (encoder hookup later)
 *   x_mm,a_deg ← computed/external (encoder hookup later)
 *   alpha      ← computed/external
 *   quality    ← derived from sensor health (degrades when sensors fail)
 *   spare      ← 0
 *
 * Cached state:
 *   The actual I²C reads happen at 100 Hz in sensor_i2c_task. This
 *   module just snapshots the most recent cached values into a frame
 *   when telemetry_task asks. This decouples the 1 kHz telemetry rate
 *   from the slower sensor sample rate without doing 1 kHz I²C.
 */
...
void sensor_pipeline_step(void) {
    ...
    cache_lock();
    {
        ...
        } /* else preserve cached last-known-good */
...
void sensor_pipeline_read(telem_frame_t *out, uint64_t now_us, uint16_t seq) {
    ...
    out->x_mm      = x;
    out->a_deg     = a;
    /* T_N: real tension sensor not yet wired (HX711 later).
       Use nominal 15 N to keep PC safety bounds happy and replay valid. */
    out->T_N       = 15.0f;
    out->rpm       = 5.0f;

    /* MPU6050 → vib_x/y/z (in g). If IMU down, last-known-good preserved. */
    out->vib_x     = snap.accel_g[0];
    out->vib_y     = snap.accel_g[1];
    out->vib_z     = snap.accel_g[2];

    /* NTC → temp_K. If thermal down, last-known-good (or room temp default). */
    out->temp_K    = snap.ntc_temp_K;

    /* INA226 → current_A. If INA226 down, last-known-good preserved. */
    out->current_A = snap.current_A;

    /* alpha: cycle progress (placeholder until winding state machine) */
    out->alpha     = fminf(1.0f, t_s / 3600.0f);

    /* quality: degrades 25% for each downed sensor.
       This propagates sensor health into the PC's anomaly detector
    ...
    out->quality   = 25.0f * (float)healthy + 17.0f;   /* 17, 42, 67, 92 */

    out->spare     = 0.0f;

    /* Set TENSION_OK / TEMP_OK / VIBRATION_OK / RPM_OK from current values */
...
#ifndef ESP_PLATFORM
/* Test-only: read the current cache directly (for host integration tests) */
