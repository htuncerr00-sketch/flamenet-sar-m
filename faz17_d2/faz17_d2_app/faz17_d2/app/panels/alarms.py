...
class AlarmsPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # Title
        title = QLabel("Alarm & Güvenlik Paneli")
        title.setProperty("role", "header")
        layout.addWidget(title)

        # Status grid: subsystem LEDs
        status_box = QGroupBox("Güvenlik Durumu")
        status_grid = QGridLayout(status_box)
        status_grid.setSpacing(6)

        self._leds = {
            "tension": LedIndicator("Gerilim", "ok"),
            "rpm":     LedIndicator("İş Mili RPM", "ok"),
            "motion":  LedIndicator("Hareket Sınırları", "ok"),
            "vib":     LedIndicator("Titreşim", "ok"),
            "thermal": LedIndicator("Termal", "ok"),
            "encoder": LedIndicator("Enkoder", "ok"),
            "estop":   LedIndicator("E-Dur Zinciri", "ok"),
            "comms":   LedIndicator("İletişim", "ok"),
        }
        row, col = 0, 0
        ...
        self._warn_card = MetricCard("UYARI", "", "uyarılar")
        self._crit_card = MetricCard("KRİTİK", "", "kritik")
        self._fatal_card = MetricCard("ÖLÜMCÜL", "", "ölümcül olaylar")
        self._halted_card = MetricCard("Durdu", "", "sistem durumu")
        ...
        self._halted_card.set_value("Hayır")
        ...
        self._estop_btn = QPushButton("⚠  ACİL DURDUR")
        ...
        self._clear_btn = QPushButton("Durmayı Temizle (arıza giderildikten sonra)")
    ...
    def on_safety_event(self, ev: SafetyEvent):
        ...
        if ev.level == SafetyLevel.WARN:
            ...
            self._halted_card.set_value("EVET")
